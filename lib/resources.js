...
export function addResourceTimings(beacon: PageLoadBeacon) {
  if (isResourceTimingAvailable && window.JSON) {
    const entries = getEntriesTransferFormat(performance.getEntriesByType('resource'));
    beacon['res'] = window.JSON.stringify(entries);
  } else if (DEBUG) {
...
export function getEntriesTransferFormat(performanceEntries: Array<Object>) {
