...
import { jsonPayloadNativelySupported } from '../transmission/index';
...
export function addResourceTimings(beacon: BeaconWithResourceTiming, minStartTime: ?number) {
  if (isResourceTimingAvailable && win.JSON) {
    ...
    if (jsonPayloadNativelySupported) {
      beacon['res'] = entries;
    } else {
...
function getEntriesTransferFormat(performanceEntries: Array<Object>, minStartTime: ?number) {
  ...
  for (let i = 0, len = performanceEntries.length; i < len; i++) {
    ...
    if (
        // some iframe cases
        lowerCaseUrl === 'about:blank' || lowerCaseUrl.indexOf('javascript:') === 0 ||
        // The image based data transmission can be visible as a resource. Do not report it.
        (lowerCaseReportingUrl != null && initiatorType === 'img' && lowerCaseUrl.indexOf(lowerCaseReportingUrl) === 0)) {
      continue;
    }
