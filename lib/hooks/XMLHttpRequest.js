  ...
  function InstrumentedXMLHttpRequest() {
    ...
    xhr.setRequestHeader = function setRequestHeader(header) {
      ...
      if (traceIdHeaderRegEx.test(header)) {
        traceHeaderSeen = true;
      }
      originalSetRequestHeader.apply(xhr, arguments);
    };

    xhr.send = function send() {
      // If traceHeaderSeen is true, then the fetch instrumentation is already
      // handling this request. This happens when a fetch polyfill is used that
      // uses XHR under the hood. Since there is no reliable way to detect if
      // fetch is polyfilled or not, we use the beacon created by the fetch
      // instrumentation and discard the XHR beacon.
      if (traceHeaderSeen) {
        ignored = true;
      }
