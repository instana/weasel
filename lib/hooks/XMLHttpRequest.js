...
import {addCorrelationHttpHeaders, addResourceTiming, captureXhrHeaders, convertHeader} from './xhrHelpers';
import {debug, info, warn} from '../debug';
import {generateUniqueId, matchesAny, now} from '../util';
  ...
  XMLHttpRequest.prototype.open = function open(method, url, async) {
    ...
    function onFinish(status) {
      ...
      if (vars.headersToCapture.length > 0) {
        captureXhrHeaders(beacon, convertHeader(xhr.getAllResponseHeaders()));
      }
