...
export function processCommand(command: any[]): any {
  ...
  case 'reportingBackends':
    if (DEBUG) {
      validateReportingBackendArray('reportingBackends', command[1]);
    }
    vars.reportingBackends = command[1];
    break;
  default:
    if (DEBUG) {
...
function validateReportingBackendArray(name, reportingBackends) {
  if (!(reportingBackends instanceof Array)) {
    return warn(name + ' is not an array. This will result in errors.');
  }

  for (let i = 0, len = reportingBackends.length; i < len; i++) {
    if (
      !reportingBackends[i] ||
      !hasOwnProperty(reportingBackends[i], 'reportingUrl') ||
      !hasOwnProperty(reportingBackends[i], 'key')
    ) {
      return warn(name + '[' + i + '] is not a ReportingBackend. This will result in errors.');
    }
  }
}
